using dotnetJs.Translator.OneOf;
using System;
using static dotnetJs.Translator.OneOf.Functions;

namespace LivingThing.Core.Frameworks.Common.OneOf
{
    public struct OneOf<T0, T1, T2> : IOneOf
    {
        ...
        public T0 AsT0 =>
            _index == 0 ?
                _value0 :
                throw new InvalidOperationException($"Cannot return as T0={typeof(T0).FullName} as result is T{_index}");
        public T1 AsT1 =>
            _index == 1 ?
                _value1 :
                throw new InvalidOperationException($"Cannot return as T1={typeof(T1).FullName}  as result is T{_index}");
        public T2 AsT2 =>
            _index == 2 ?
                _value2 :
                throw new InvalidOperationException($"Cannot return as T2={typeof(T2).FullName}  as result is T{_index}");
