...
namespace NetJs.Translator.CSharpToJavascript
{
    public partial class TranslatorSyntaxVisitor
    {
        ...
        bool TryInvokeMethodOperator(CSharpSyntaxNode node, string _operator, ITypeSymbol? leftOperandType, ExpressionSyntax? leftOperand, IEnumerable<ExpressionSyntax> arguments, Action? prologue = null)
        {
            ...
            if (leftOperandType is ITypeSymbol ts)
            {
                ...
                if (operatorMethod is IMethodSymbol ms && ms.CanInvoke(_global))
                {
                    prologue?.Invoke();
                    mprologue?.Invoke();
            ...
            {
                //(double)long
                //if we get here with, its because we cant find an explicit operator on both lhs(double) and rhs(long) that satisfy this conversion
